...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS uploading (
...
def upload_file():
  if 'user_email' in session:
    if request.method == 'POST':
        ...
        conn.execute("INSERT INTO uploading (email, filename, filepath) VALUES (?, ?, ?)",
...
def my_reports():
    ...
    files = conn.execute("SELECT * FROM uploading WHERE email = ?", (email,)).fetchall()
    conn.close()
